...
const mapPageToEventAlbum = (album: PageAlbum): EventAlbum => {
  return {
    ...
    photos: album.photos?.data
      ?.map((photo) => photo.webp_images?.[0]?.source)
      .filter(Boolean) as string[],
    thumbnails: album.photos?.data
      ?.map(
        (photo) => photo.webp_images?.[photo.webp_images.length - 1]?.source,
      )
      .filter(Boolean) as string[],
...
export const getEventAlbumsServer = async (): Promise<{
  ...
  const albums = albumsData
    .filter((album) => album.type === "normal")
    .sort((a, b) => {
